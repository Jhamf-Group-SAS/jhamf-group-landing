                    ...
                    {['All', 'Cloud', 'Automation'].map((f) => (
                        <button
                            ...
                            className={`px-6 py-2 rounded-full text-sm font-medium transition-all ${filter === f
                                ? 'bg-azure-DEFAULT text-white shadow-lg shadow-azure-DEFAULT/25'
                    ...
                    <AnimatePresence>
                        {filteredCases.map((project, idx) => (
                            <motion.div
                                ...
                                key={idx}
                                className="bg-gradient-to-br from-white/5 to-white/[0.02] border border-white/10 rounded-2xl p-8 hover:border-azure-DEFAULT/30 transition-colors group"
                            >
                                <div className="flex justify-between items-start mb-6">
                                    <div>
                                        <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium mb-3 ${project.type === 'Cloud'
                                            ? 'bg-azure-DEFAULT/10 text-azure-DEFAULT border border-azure-DEFAULT/20'
                                            : 'bg-neon-purple/10 text-neon-purple border border-neon-purple/20'
                                            }`}>
                                            {project.type === 'Cloud' ? t('grid.filters.cloud') : t('grid.filters.automation')}
                                        </span>
                                        <h3 className="text-2xl font-bold text-white group-hover:text-azure-DEFAULT transition-colors">
                                            {project.title}
                                        </h3>
                                        <p className="text-sm text-gray-500 mt-1">{project.client}</p>
                                    </div>
                                    <div className="p-2 bg-white/5 rounded-full group-hover:bg-azure-DEFAULT group-hover:text-white transition-all overflow-hidden relative">
